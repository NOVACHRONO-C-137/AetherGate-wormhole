...
const evmListener_js_1 = require("./evmListener.js");
const solanaBroadcaster_js_1 = require("./solanaBroadcaster.js");
...
// ─── Retry queue ─────────────────────────────────────────────────────────────
const retryQueue = [];
async function scheduleRetry(event, relayFn) {
    if (event.retriesLeft <= 0) {
        logger_js_1.logger.error(`[Relay] DEAD LETTER — nonce ${event.bridgeNonce} exhausted retries. Manual review required.`);
        return;
    }
    const delay = 5000 * (6 - event.retriesLeft); // backoff
    event.retriesLeft--;
    logger_js_1.logger.warn(`[Relay] Retrying nonce ${event.bridgeNonce} in ${delay}ms (${event.retriesLeft} left)`);
    await sleep(delay);
    await relayFn();
}
// ─── Relay dispatcher ────────────────────────────────────────────────────────
async function createDispatcher(config, signer, evmListener, solanaBroadcaster) {
    return async (event) => {
        const relay = async () => {
            try {
                if (event.direction === types_js_1.BridgeDirection.EVM_TO_SOLANA) {
                ...
                else if (event.direction === types_js_1.BridgeDirection.SOLANA_TO_EVM) {
                    if (event.eventType === "LOCK") {
                        // Solana Lock → EVM mintWrapped
                        const foreignAssetId = event.assetId.startsWith("0x")
                            ? event.assetId
                            : "0x" + Buffer.from(bs58_1.default.decode(event.assetId)).toString("hex").padStart(64, "0");
                ...
                logger_js_1.logger.error(`[Relay] Error processing nonce ${event.bridgeNonce}:`, err);
...
async function main() {
    ...
    logger_js_1.logger.info("   AetherGate Relayer v1.0 starting    ");
    logger_js_1.logger.info("═══════════════════════════════════════");
    const config = loadConfig();
    const signer = new signer_js_1.SigningModule(config.relayerEvmPrivateKey, 11155111); // Sepolia
    // Create listeners with a temporary no-op callback; we'll set the real dispatcher after creation
    let dispatcher;
    const evmListener = new evmListener_js_1.EvmListener(config, async (event) => {
        await dispatcher(event);
    });
    const solanaBroadcaster = new solanaBroadcaster_js_1.SolanaBroadcaster(config, async (event) => {
        await dispatcher(event);
    });
    ...
    logger_js_1.logger.info("[Main] Relayer running. Listening for cross-chain events...");
}
function shutdown(evm, sol) {
    logger_js_1.logger.info("[Main] Shutting down gracefully...");
    evm.stop();
    sol.stop();
    process.exit(0);
}
...
main().catch((e) => {
    logger_js_1.logger.error("Fatal relayer error:", e);
    process.exit(1);
});
